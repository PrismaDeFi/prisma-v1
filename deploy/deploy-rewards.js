...
module.exports = async ({ deployments }) => {
  ...
  const prismaToken = await deploy("MockPrismaToken", {
    from: deployer.address,
    args: [],
    log: true,
  })
  ...
  const prismaRewards = await deploy("PrismaRewards", {
    from: deployer.address,
    args: [prismaToken.address, busd.address, router.address],
    log: true,
  })
  log(`PrismaRewards deployed at ${prismaRewards.address}`)
}
