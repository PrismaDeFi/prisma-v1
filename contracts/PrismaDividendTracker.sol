...
contract PrismaDividendTracker is DividendPayingToken {
  ...
  function getPrismaPair() external view returns (address) {
    ...
    return IUniswapV2Factory(factory).getPair(prismaToken, dividendToken);
  }

  function addLiquidity(uint256 prismaAmount, uint256 busdAmount) external {
    IERC20(prismaToken).approve(address(uniswapV2Router), prismaAmount);
    IERC20(dividendToken).approve(address(uniswapV2Router), busdAmount);
    router.addLiquidity(
      prismaToken,
      dividendToken,
      prismaAmount,
